...
describe('PlayerStore', () => {
  ...
  it('keeps the image buffer synced up', () => {
    const ps = new PlayerStore(2, 2);
    ps.set(1, { x: 0, y: 0, color: new Buffer([0x01, 0x02, 0x03, 0x04]) });
    ps.set(2, { x: 1, y: 0, color: new Buffer([0x05, 0x06, 0x07, 0x08]) });
    ps.set(3, { x: 0, y: 1, color: new Buffer([0x09, 0x0a, 0x0b, 0x0c]) });
    ps.set(4, { x: 1, y: 1, color: new Buffer([0x0d, 0x0e, 0x0f, 0xff]) });
    ...
    ps.set(2, { x: 1, y: 0, color: new Buffer([0x05, 0x06, 0x07, 0x09]) });
    expect(ps.imageBuffer).to.deep.equal(new Buffer([
      0x01, 0x02, 0x03, 0x04,
      0x05, 0x06, 0x07, 0x09,
      0x09, 0x0a, 0x0b, 0x0c,
    ...
    ps.set(5, { x: 1, y: 0, color: new Buffer([0xfd, 0xf0, 0xfa, 0xf1]) });
    expect(ps.imageBuffer).to.deep.equal(new Buffer([
      0x01, 0x02, 0x03, 0x04,
      0xfd, 0xf0, 0xfa, 0xf1,
      0x09, 0x0a, 0x0b, 0x0c,
